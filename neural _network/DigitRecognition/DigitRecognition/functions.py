...
def initWB(layers_dims):
    ...
    for l in range(1, L):
        ...
        parametrs["b" + str(l)] = np.zeros((layers_dims[l], 1))
    return parameters

#Функция записи параметров в файл
def inputWB(parameters):
    count = 0
    while parameters.get("W" + str(count)) != None:
        file = open('W' + str(count) + '.txt', 'a')
        for i in parameters["W" + str(count)]:
            file.write(' '.join(i))
        file.close()
        count += 1
    count = 0
    while parameters.get("b" + str(count)) != None:
        file = open('b' + str(count) + '.txt', 'a')
